...
| ------------- | ------------------- | ---------------- | -------- | ------ |
| Restrictions | when the question holds part of the specification below what it admits | per restriction, the quantity it holds and the set it is held to, or, where no single quantity carries it, a name and the expression it holds under; a restriction naming a variable also states how it is held — clamped · restricted · initial · stationary · without-event — and each carries a justification saying why the question is put only under it | connection probability between 0.05 and 0.2; the velocity field held stationary; more trainable parameters than training examples | target population [lundberg2021] |
| Limit | only when the question concerns the limit system itself | the expression "[quantity] → [limit value]", with a justification stating why the limit system is what the question is about | network width → ∞ (kernel regime); number of spins $N\to\infty$ (thermodynamic limit) | limit systems [norton2012] |
