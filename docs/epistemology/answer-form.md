...
- **An answer may explain by the environment, and then names the external system on which it draws.** The external systems pass the [inclusion test](subject-of-inquiry.md#^inclusion-test) of the subject of inquiry. An answer may therefore explain the phenomenon by the structure of the inputs, or by the class of system that reads the outputs. Each factor that draws on the environment names the external system involved. The appeal then resolves to an [external system](target-system.md) that the target declares. The answer reaches each quantity of that external system as a term under its own symbol, and never declares it a second time. ^explanans-appeals
