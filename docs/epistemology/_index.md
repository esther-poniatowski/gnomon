...
These notes turn results from the philosophy of models, of explanation and of questions into a form that specifies a research inquiry. One note separates the layers of an inquiry, and each of the others develops one layer or one component. A research project applies the method by filling one [form per target system, phenomenon, question and candidate answer](../../src/gnomon/data/templates/_index.md).

- [Specifying an inquiry](inquiry-specification.md): separates the layers of specification, from what the inquiry is about to the form of its answer, so that each layer is fixed independently. The note also states the four goals that the specification serves.
- [Subject of inquiry](subject-of-inquiry.md): fixes what a research question is about, and states the rule that admits a specification into each of its four components. A question arises only once an epistemic task is added to its subject.
    - [Target system](target-system.md): specifies the system studied as a schema of variables, parameters and laws. The instances of that schema form the reference class.
    - [Phenomenon](phenomenon.md): specifies the behavior of the target system that the question concerns, down to the variable that expresses it and the grain at which it is described.
    - [Contrast](contrast.md): specifies the alternatives against which the phenomenon is questioned; the choice of alternatives decides which cause counts as explanatory.
    - [Domain of conditions](domain.md): records the restrictions of the a priori specification of the system that the question holds fixed.
- [Epistemic task](epistemic-task.md): fixes what kind of knowledge the answer must deliver, and thereby the interrogative word of the question. The modal strength of the answer is specified in this note directly.
