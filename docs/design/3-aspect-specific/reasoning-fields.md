...
> **Tier 3 (per aspect).** Instantiates the architectural commitments at [reasoning-annotation fields](../2-architecture/reasoning-structure.md#^t2-reasoning-annotation-fields) (resolved as fields that vary by profile), [reasoning-annotation attachment](../2-architecture/reasoning-structure.md#^t2-reasoning-annotation-attachment) (the grid of locus × content kind), and [partial-formalization profiles](../2-architecture/granularity.md#^t2-partial-formalization-profiles) (the profile shape). This file fixes what the architecture deferred: the field set and field type for each cell, the named profiles, and the chain of guarantees.
...
- **Minimal spine with prose defaults.** Each cell has a small mandatory spine of fields and admits a prose body for the rest. The spine fields are mandatory at every formalization profile; nothing else is mandatory. Content by cell:
	- *Canonical, licensing*: `content`, `applied_schema_id` (reference to [operation-schema placement](../2-architecture/operations-and-modes.md#^t2-operation-schema-placement)), `support_edges` (each carrying `warrant_kind` per [warrant-kind annotation](../2-architecture/validity-revision.md#^t2-warrant-annotation)).
  ...
  *Tradeoffs.* Authors write the least extra structure ([t1-feasibility](../1-framework/cost-ergonomics.md#^t1-system-scale), [t1-partial-formalization](../1-framework/expressive-depth.md#^t1-partial-formalization)). Queries and validators see little structure; the "five specifications per step" required by [t1-non-arbitrary](../1-framework/reasoning-integrity.md#^t1-served-goal) cannot be checked mechanically because four specifications live in prose.
...
- **Structured spine with fields that vary by profile.** Each cell has a mandatory spine *and* a structured optional layer. The formalization profile decides which optional fields become mandatory. Content by cell:
	...
	- *Assembly, explanatory*: a structured record — `gain_kind` (controlled enum, exact vocabulary still open), `gain_statement` (free text).
		...
		- *Assembly, promotable*: structure depends on the kind — internal lemma as `{statement, support_edges}`; named rejected route as `{label, failure_kind, rationale}` (the `failure_kind` enum comes from the [revision vocabulary](revision-vocabulary.md)); support endpoint inside an assembly as `{statement, warrant_kind}`.
	  A formalization profile (per [partial-formalization profiles](../2-architecture/granularity.md#^t2-partial-formalization-profiles)) selects which optional fields become mandatory. The spine is mandatory at every profile. The strongest profile makes every optional field mandatory; that is what [t1-intelligibility](../1-framework/reasoning-integrity.md#^t1-justification-levels) and [t1-non-arbitrary](../1-framework/reasoning-integrity.md#^t1-served-goal) would impose if read as hard requirements. This profile does not fit informal research practice, so it forms the strict pole of tension [X2](../1-framework/_tensions.md#^t2-x2).

	  *Tradeoffs.* Queries and validators see enough structure to check [t1-non-arbitrary](../1-framework/reasoning-integrity.md#^t1-served-goal) when the profile demands it. Authors write more annotations, but profiles keep that burden adjustable. The schema becomes heavier and relies on [partial-formalization profiles](../2-architecture/granularity.md#^t2-partial-formalization-profiles) to partition mandatory and optional fields.
...
- **Structured canonical cells with prose assembly cells.** Field requirements differ by locus:
	...
	- *Assembly cells*: prose only, no required structure (strategic, explanatory, process, and promotable kinds all default to prose), with the schema declaring only the *presence* of each kind, not its internal structure. Promotable records become structured at promotion time.
	  ...
	  *Tradeoffs.* Canonical content is durable and shared, so it gets typed; assembly content belongs to an inquiry, so it stays loose. This preserves [non-redundancy](../1-framework/modular-content-organization.md#^t1-non-redundancy) for canonical records and [t1-feasibility](../1-framework/cost-ergonomics.md#^t1-system-scale) for assemblies. It loses query support over assemblies and weakens [t1-intelligibility](../1-framework/reasoning-integrity.md#^t1-justification-levels) at the strategic and explanatory levels.
...
Unspecified. The enum must be drafted and checked against the [revision rationale enum](revision-vocabulary.md) and the [warrant-kind enum](warrant-vocabulary.md) so that the three vocabularies partition cleanly rather than overlap (per [t3-vocabulary-partition](revision-vocabulary.md#^t3-vocabulary-partition)).
...
Settled at [partial-formalization profiles](../2-architecture/granularity.md#^t2-partial-formalization-profiles).
