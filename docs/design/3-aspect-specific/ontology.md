...
A candidate becomes a canonical object kind only if it satisfies all five conditions of [the object-kind admission test](../2-architecture/object-kinds.md#^t2-object-kind-admission): independent identity, context-transcendent reuse, local validity, independent editability, irreducibility.
...
The ontology begins small, allows subtypes gradually under the [subtype discipline](../2-architecture/object-kinds.md#^t2-subtype-discipline) open question, and is itself versioned so that taxonomic restructuring is recordable.

---

## Decisions

*To be drafted at the ontology work in [the project TODO](../../TODO.md).*
...
> [!QUESTION] What are the admissible subtypes of the unified Question/Goal kind ratified in [t2-question-vs-goal](../2-architecture/object-kinds.md#^t2-question-vs-goal), and what distinguishes them?
...
Candidate subtypes (to be evaluated under [t3-admissibility-five-conditions](#^t3-admissibility-five-conditions) and [t2-subtype-discipline](../2-architecture/object-kinds.md#^t2-subtype-discipline)):
...
Bearing criteria: [t3-admissibility-five-conditions](#^t3-admissibility-five-conditions), [t2-subtype-discipline](../2-architecture/object-kinds.md#^t2-subtype-discipline), [object-kind set smallness](../2-architecture/object-kinds.md#^t2-ontology-small), [functional separation of concerns](../1-framework/framework-foundations.md#^t1-functional-separation).

Downstream consequence: the chosen subtype set bounds the *Strategic*-level `gap` vocabulary in [recoverable reasoning](../1-framework/reasoning-integrity.md#^t1-justification-levels). It also feeds the strategic-game commitment at [inquiry content and progression](../1-framework/framework-foundations.md#^t1-inquiry-content-and-progression).
...
The taxonomy cuts at the **content-referent level** (what a definition or claim refers to), not at the epistemic-kind level (claim, definition, proof) that [object kinds](../2-architecture/object-kinds.md) governs. It is a candidate sub-categorization *within* content kinds.
...
>
> - **Distinct kind.** The placeholder is its own object kind, admitted or rejected under [the object-kind admission test](../2-architecture/object-kinds.md#^t2-object-kind-admission).
> - **Maturity state.** The placeholder is a content object (a definition or concept) at an `undefined`/`stub` status; it folds into [t1-partial-formalization](../1-framework/expressive-depth.md#^t1-partial-formalization) and the per-kind status enums at [t2-epistemic-status](../2-architecture/object-kinds.md#^t2-epistemic-status). This option converges with the pre-formal `Idea` kind question staged in [fleeting-ideas](../_fleeting-ideas.md#^fleeting-kind-set) and with the four-way proof-status distinction staged in [fleeting-ideas](../_fleeting-ideas.md#^fleeting-proof-status).
...
A second expert review of the motivating worked example contributed five refinements to the normal form, recorded here. The undischarged-commitment refinements it also produced — presuppositions, definitional force, commitment strength — are filed separately as [the undischarged-commitment open question](arguments-reasoning.md#^t3-undischarged-commitments), since they bear on silent incompleteness rather than on definition structure.

**Three separable operations.** The framework should not conflate three operations on a definition: *normalization* makes the internal roles explicit and is possible immediately; *definition completion* fully defines every referenced term and stays optional; *validation* checks that all references are well-formed and coherent and is delayed. A statement is accepted once normalized, before completion or validation. Bears on [t1-validation-externality](../1-framework/research-activities-workflows.md#^t1-validation-externality) and [t1-write-side-automation](../1-framework/cost-ergonomics.md#^t1-write-side-automation).
...
The template registry connects to the reusable [reasoning schemes](arguments-reasoning.md#^t3-reasoning-schemes): a definition template is the definition-side analogue of an argument scheme.
...
- **Resolution-policy field**, a three-valued enum stating whether and why the set will narrow:
  ...
  - `will-not-resolve` — the term is kept plural by an authored choice because the ambiguity carries cognitive value; resolution is declined, not deferred. A *time-bounded* sub-case ("plural for now") sets `will-not-resolve` with a review date. *Validator:* the canonicity check at [t1-canonical-terminology](../1-framework/modular-content-organization.md#^t1-canonical-terminology) must **exempt** a term marked `will-not-resolve` rather than flag it as drift. *Worked examples*: [a term that is intentionally overloaded](../_worked-examples.md#^example-statement-where-a-term-is-intentionally-overloaded) and [a statement whose role is to preserve ambiguity](../_worked-examples.md#^example-statement-whose-role-is-to-preserve-ambiguity).

The earlier `defective` status is **not an ambiguity status** and is removed from this field. A concept being defectively ambiguous — an overload or an unstable boundary that should be repaired — is an authored *claim about the concept*, not a state of its candidate set: it asserts the concept *ought to change*, where the resolution policy only records whether its candidate set *will* change. The defect claim is a first-class object connecting to the concept-as-object reading at [fleeting-ideas](../_fleeting-ideas.md#^fleeting-concept-metamodeling); `[!missing]` tension callouts on [t1-canonical-terminology](../1-framework/modular-content-organization.md#^t1-canonical-terminology) and [functional separation of concerns](../1-framework/framework-foundations.md#^t1-functional-separation) record that a defect claim is a recordable boundary failure rather than a term to silently rename. *Worked example*: [a statement whose content is a failure mode of a concept](../_worked-examples.md#^example-statement-whose-content-is-a-failure-mode-of-a-concept).

This handling of content concepts with unstable boundaries is what the [functional separation of concerns](../1-framework/framework-foundations.md#^t1-functional-separation) criterion defers here: that criterion governs the boundaries of the framework's *own components* (object kinds, fields, relations, vocabularies), not the concepts the inquiry manipulates. A content concept kept deliberately plural (`will-not-resolve`) or diagnosed as defective is handled by the candidate-set machinery above, not by the criterion.

**Stable cross-statement placeholders, and the lexical/concept separation.** A placeholder is stable and shared: a later statement referencing the same term binds to the same placeholder, so resolving it once resolves it everywhere. This requires distinguishing five levels that prose conflates — the *lexical term* (the surface string), the *unresolved concept candidate* (the term used but not yet resolved to a concept), the *resolved concept*, the *local role* the term plays in a given statement, and the *global ontology class*. A term occurrence records its surface, its span, its local role, and its candidate referent. This converges with the concept-as-class versus concept-as-object distinction at [fleeting-ideas](../_fleeting-ideas.md#^fleeting-concept-metamodeling), and tensions mildly with [t1-canonical-terminology](../1-framework/modular-content-organization.md#^t1-canonical-terminology): a term may carry an *unresolved* candidate referent before it has a designated canonical concept.
...
**External precedents.** The normalization machinery has established precedents, recorded as citations rather than as candidates: frame semantics (a term evokes a structured frame with roles), typed feature structures (partially specified objects with typed attributes supporting underspecification), Abstract Meaning Representation (role-explicit sentence graphs keeping the original wording separate), and semantic-annotation systems (prose stays intact while structured annotations attach to spans). Description Logic with placeholders is a possible *export target* once holes are resolved, not an early-stage representation — consistent with the DL-review rejections at [fleeting-ideas](../_fleeting-ideas.md#^fleeting-rejections).

Bearing criteria: [t1-typed-object-decomposition](../1-framework/framework-foundations.md#^t1-typed-object-decomposition), [functional separation of concerns](../1-framework/framework-foundations.md#^t1-functional-separation), [t1-no-infinite-regress](../1-framework/expressive-depth.md#^t1-no-infinite-regress), [t1-partial-formalization](../1-framework/expressive-depth.md#^t1-partial-formalization).

The worked example that motivates this open question, and a candidate normalization pipeline and object model, are recorded at [worked-examples](../_worked-examples.md#^example-genus-differentia).

### Scope of terminology canonicity ^t3-canonicity-scope

> [!QUESTION] Is terminology canonicity global across the whole framework, or local to a project or module?

[t1-canonical-terminology](../1-framework/modular-content-organization.md#^t1-canonical-terminology) commits the framework to canonical terminology — each concept has a designated name, automatically enforced. That commitment does not fix the **scope** over which canonicity holds:

- **Global** — one canonical name per concept across the entire corpus.
- **Module-local** — canonicity holds within a project or module, drawn from Description-Logic ontology modules. The same term may then legitimately carry different definitions in a neuroscience module and a deep-learning-theory module.

A staged candidate for the module-local option is recorded at [fleeting-ideas](../_fleeting-ideas.md#^fleeting-ontology-modularity). Resolution must state which scope holds and, if module-local, how cross-module aliasing is linked — so that a term canonical in one module can be related to its counterpart in another. The choice bears on the terminology-enforcement validator, whose home is the validation-views theme.

Bearing criteria: [t1-canonical-terminology](../1-framework/modular-content-organization.md#^t1-canonical-terminology).
