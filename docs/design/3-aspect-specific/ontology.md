...
The ontology begins small, allows subtypes gradually under the [subtype discipline](vendor/gnomon/docs/design/2-architecture/object-kinds#^t2-subtype-discipline) open question, and is itself versioned so that taxonomic restructuring is recordable.
...
Bearing criteria: [t3-admissibility-five-conditions](#^t3-admissibility-five-conditions), [t2-subtype-discipline](vendor/gnomon/docs/design/2-architecture/object-kinds#^t2-subtype-discipline), [object-kind set smallness](vendor/gnomon/docs/design/2-architecture/object-kinds#^t2-ontology-small), [functional separation of concerns](../1-framework/framework-foundations#^t1-functional-separation).

Downstream consequence: the chosen subtype set bounds the *Strategic*-level `gap` vocabulary in [recoverable reasoning](../1-framework/reasoning-integrity#^t1-justification-levels). It also feeds the strategic-game commitment at [inquiry content and progression](../1-framework/framework-foundations#^t1-inquiry-content-and-progression).
...
> - **Maturity state.** The placeholder is a content object (a definition or concept) at an `undefined`/`stub` status; it folds into [t1-partial-formalization](../1-framework/expressive-depth#^t1-partial-formalization) and the per-kind status enums at [t2-epistemic-status](vendor/gnomon/docs/design/2-architecture/object-kinds#^t2-epistemic-status). This option converges with the pre-formal `Idea` kind question staged in [fleeting-ideas](vendor/gnomon/docs/design/_fleeting-ideas#^fleeting-kind-set) and with the four-way proof-status distinction staged in [fleeting-ideas](vendor/gnomon/docs/design/_fleeting-ideas#^fleeting-proof-status).
...
[t1-canonical-terminology](../1-framework/modular-content-organization#^t1-canonical-terminology) commits the framework to canonical terminology — each concept has a designated name, automatically enforced. That commitment does not fix the **scope** over which canonicity holds:
