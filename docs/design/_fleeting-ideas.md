...
**Linking-phrase family taxonomy.** A candidate classification of linking phrases by **epistemic function** — which satisfies [t1-function-driven-typology](1-framework/framework-foundations#^t1-function-driven-typology) (classify by function, not grammatical form):
...
| Rust frozen-dataclass / revision-produces-new-object discipline as a new proposal | statement-normalization review | Restates ratified state — [t1-revision-accountability](1-framework/research-activities-workflows#^t1-revision-accountability) and [t1-git-delegation](1-framework/external-interfaces#^t1-git-delegation) already commit to revision as an authored edit creating a new state. The Rust ownership formalism is kept separately as [ownership and borrowing](#^fleeting-ownership-borrowing). |
