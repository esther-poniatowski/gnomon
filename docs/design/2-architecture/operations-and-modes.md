...
>
> Framework-level criteria bearing on this file: [no infinite regress](../1-framework/expressive-depth.md#^t1-no-infinite-regress) — the requirement that reasoning-description terminate, which the closed operational core implements — and [reasoning-types coverage](../1-framework/expressive-depth.md#^t1-reasoning-types-coverage). The meta-schema rule [field-typing discipline](object-kinds.md#^t2-field-typing) binds the operation-schema fields declared here. See also tension [X1](../1-framework/_tensions.md#^t2-x1).
...
> The placement is the same under all primitiveness regimes ([t2-operation-primitiveness](#^t2-operation-primitiveness) below); only the contents differ. Per-alternative contents and constructor proposals: see [Operation schemas](../3-aspect-specific/operation-schemas.md).
...
> Which fields and validators are mandatory in each mode, and what degrades under partial specification, is deferred to [t2-partial-formalization-profiles](granularity.md#^t2-partial-formalization-profiles).
...
- **Schema calculus** — a small set of primitive constructors from which domain schemas are derived (constructor proposals in [Operation schemas](../3-aspect-specific/operation-schemas.md)).
- **Open library** — implicitly ruled out by all sources, recorded as a pole.

Bearing criteria: [no infinite regress](../1-framework/expressive-depth.md#^t1-no-infinite-regress), [t1-expressivity](../1-framework/expressive-depth.md#^t1-reasoning-types-coverage), tension [X1](../1-framework/_tensions.md#^t2-x1). Tension X1 favors deriving schemas from a small calculus.
...
Bearing criteria: [revision accountability](../1-framework/research-activities-workflows.md#^t1-revision-accountability), [t1-non-arbitrary](../1-framework/reasoning-integrity.md#^t1-served-goal).
