...
**Canonical object vs. assembly.** Both are persistent records in the source layer with their own identifiers and typed relations. They differ by which clauses of [t2-object-kind-admission](vendor/gnomon/docs/design/2-architecture/object-kinds#^t2-object-kind-admission) they satisfy:
...
| --- | --- | --- |
| **Licensing / inferential support** | Canonical object | content, status (per [t2-epistemic-status](vendor/gnomon/docs/design/2-architecture/object-kinds#^t2-epistemic-status)), applied operation schema, warrant kind on its support relations (per [t2-warrant-annotation](vendor/gnomon/docs/design/2-architecture/validity-revision#^t2-warrant-annotation)), preconditions |
...
- citations of canonical objects with typed relations and warrant kind on support edges (per [t2-warrant-annotation](vendor/gnomon/docs/design/2-architecture/validity-revision#^t2-warrant-annotation));
...
**Promotion rule.** An assembly-local record is promoted to the canonical store when it satisfies [t2-object-kind-admission](vendor/gnomon/docs/design/2-architecture/object-kinds#^t2-object-kind-admission). The trigger is a property of the record, not of the inquiring author's intent.
...
> - **When promotion triggers.** The predicates in the admission test are stated at the criterion level in [t2-object-kind-admission](vendor/gnomon/docs/design/2-architecture/object-kinds#^t2-object-kind-admission). How the validator checks those predicates is deferred to [t2-validator-placement](vendor/gnomon/docs/design/2-architecture/validation-views#^t2-validator-placement).
