...
>
> Theme-local criterion bearing on this file: [layer replaceability](layering.md#^t2-layering-replaceability) — validator implementations must be replaceable without revising the rules. Framework-level criteria bearing on this theme: [read-side automation](../1-framework/cost-ergonomics.md#^t1-read-side-automation) and [write-side automation](../1-framework/cost-ergonomics.md#^t1-write-side-automation) — the mechanizability the validators realise — and [activity separation](../1-framework/research-activities-workflows.md#^t1-activity-access-rights).
...
- **Validation rules belong to the schema layer** (the *what*: invariants any object, relation, or assembly must satisfy). The schema declares what counts as well-formed. This satisfies [write-side automation](../1-framework/cost-ergonomics.md#^t1-write-side-automation): validation is mechanizable without human judgment.
...
> Validation rules must cover both canonical objects and assemblies (per [locus of justificatory annotations](reasoning-structure.md#^t2-reasoning-annotation-attachment), [assemblies relative to a target](reasoning-structure.md#^t2-target-relative-assemblies)). Their structural commitments differ:
>
...
>
> [promotion of assembly-local records](reasoning-structure.md#^t2-assembly-record-promotion) fixes what an assembly may contain; the schema layer must declare:
>
...
>
> Validators flag promotion candidates per [t2-build-vs-mutation](layering.md#^t2-build-vs-mutation); they do not move records. The exact field set within each kind depends on [t2-reasoning-annotation-fields](reasoning-structure.md#^t2-reasoning-annotation-fields).
...
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Schema enforcement**       | Checks required-field and field-typing per kind                                                                         | Field name drift, missing required fields, type errors ([field-typing discipline](object-kinds.md#^t2-field-typing))                                                                               |
| **?**                        | Checks relation-edge typing, vocabulary closure                                                                         | ([build-error policy](relations-graph.md#^t2-relation-storage-locus)) ==TODO: appropriate ref?==                                                                                                   |
...
| **Graph integrity**          | Detects cycle on proof-dependency edges                                                                                 | Circular dependencies introduced silently ([snapshot acyclicity](relations-graph.md#^t2-snapshot-dag-property), [no circular reasoning](../1-framework/reasoning-integrity.md#^t1-vl-no-circular)) |
...
| **Archival integrity**       | Ensures archived records are well-formed with revision-object pairing                                                   | ([t2-archival](validity-revision.md#^t2-archival))                                                                                                                                                 |
| **Terminology enforcement**  | Lints prose against the canonical terminology                                                                           | Naming drift across notes ([t1-canonical-terminology](../1-framework/modular-content-organization.md#^t1-canonical-terminology))                                                                   |
...
Bearing criteria: [t1-read-side-automation](../1-framework/cost-ergonomics.md#^t1-read-side-automation), [t1-write-side-automation](../1-framework/cost-ergonomics.md#^t1-write-side-automation), [t1-validation-externality](../1-framework/research-activities-workflows.md#^t1-validation-externality).
