...
The complete equilibrium state and the dimension of the Jacobian depend on the target variant. [The operators that this problem declares](operators.yml) must therefore leave three types open:
...
[The consumer-resource theorem](answer.yml) supplies one of those narrower results. The theorem [passes the sufficient proof obligation for the stated subclass](assessment.yml) and fails three tests:
...
The answer can name the target variant `community with explicit resources`, but it cannot declare the narrower domain of the theorem: communities whose microbes consume resources without producing any. The hypotheses of the theorem must therefore be distributed across a law expression and explanans conditions. A distinct field for the domain of a claim is recorded as a need among [the defects shared by the answer and assessment templates](../answer-assessment-template-issues.md).
