...
The one-dimensional generalized Swift-Hohenberg equation admits stationary localized patterns organized by homoclinic snaking. Within the admitted coefficient domain, what spatial and bifurcation structure characterizes these states?

## Formal target

The target is the generalized Swift-Hohenberg equation with quadratic and cubic nonlinearities on the real line. Its stationary localized states approach the homogeneous solution at spatial infinity. The target is the formal model itself, so the inquiry requires no external physical system.
...
- fix the phenomenon without fixing the values of the features sought;
- identify the features that a complete description must cover;
